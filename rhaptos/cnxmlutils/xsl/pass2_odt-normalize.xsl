...
<!-- remove empty para, headers, and spans. -->
  ...
  <xsl:template match="text:span[count(node())=0 and not(@png_filename)]" />
...
<xsl:template match="@png_filename|@fo:font-style|@fo:font-weight|@style:text-underline|@style:text-underline-style|@style:text-position|@style:parent-style-name|@fo:color[translate(.,'ABCDEF','abcdef') = '#ff0000']" mode="stylizer">
