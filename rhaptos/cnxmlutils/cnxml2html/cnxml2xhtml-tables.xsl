...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" 
	  ...
	  <table cellspacing="0" cellpadding="0">
		...
		<xsl:choose>
		  ...
		  <xsl:when test="@frame='topbot'">
            ...
            <xsl:if test="c:name[node()] or                            c:title[node()] or                            c:caption[node()] or                            c:label[node()] or                            (not(c:label[not(node())]) and                             not(ancestor::*[1][self::c:figure or self::c:subfigure]))">
              <caption>
                <xsl:if test="c:label[node()] or                                (not(c:label[not(node())]) and                                 not(ancestor::*[1][self::c:figure or self::c:subfigure]))">
                  ...
                  <span class="cnx-gentext-caption cnx-gentext-n">
                    <xsl:if test="not(ancestor::*[1][self::c:figure or self::c:subfigure])">
                      <xsl:for-each select="ancestor::c:chapter">   
                      ...
                      <xsl:choose>
                        <xsl:when test="@type and $type!='table'">
                          <xsl:number level="any" from="c:preface|c:chapter" count="c:table[not(ancestor::*[1][self::c:figure or self::c:subfigure])]" format="1. "/>
                        </xsl:when>
                        <xsl:otherwise>
                          <xsl:number level="any" from="c:preface|c:chapter" count="c:table[not(ancestor::*[1][self::c:figure or self::c:subfigure])][not(@type) or @type='table']" format="1. "/>
                        </xsl:otherwise>
                      </xsl:choose>
                    </xsl:if>
                  </span>
                ...
                <xsl:if test="c:caption[node()]">
                  <xsl:variable name="caption-element">
                    <xsl:choose>
                      <xsl:when test="c:name[node()] or c:title[node()]">div</xsl:when>
                      <xsl:otherwise>span</xsl:otherwise>
                    </xsl:choose>
                  </xsl:variable>
                  <xsl:element name="{$caption-element}">
                    <xsl:attribute name="class">table-caption caption</xsl:attribute>
                    <xsl:if test="c:caption/@id">
                      <xsl:attribute name="id">
                        <xsl:value-of select="c:caption/@id"/>
                      </xsl:attribute>
                    </xsl:if>
	    ...
	    <xsl:choose>
	      <xsl:when test="count(c:tgroup) &gt; 1">
		<tbody>
		  <xsl:for-each select="c:tgroup">
		    <tr>
			...
			<table cellspacing="0" cellpadding="0" width="100%" style="border: 0 !important; margin: 0 !important;">
	    		  <xsl:apply-templates select="self::c:tgroup"/>
			</table>
		      </td>
		    </tr>
		  </xsl:for-each>
  ...
  <xsl:template match="c:thead|c:tfoot|c:tbody">
	...
	<xsl:choose>
	  ...
	  <xsl:otherwise>
	    <xsl:choose>
	      <xsl:when test="self::c:tbody or self::c:tfoot">
		<xsl:text>top</xsl:text>
	      </xsl:when>
	      <xsl:when test="self::c:thead">
  ...
  <xsl:template match="c:thead/c:row/c:entry|c:tfoot/c:row/c:entry">
  ...
  <xsl:template match="c:tbody/c:row/c:entry">
  ...
  <xsl:template name="row.header.or.not">
    ...
    <xsl:choose>
      ...
      <xsl:when test="substring(ancestor::*[3]/c:spanspec[@spanname=current()/@spanname]/@namest,1,7) = 'header_'">th</xsl:when>
      <!-- nearest entrytbl/colspec or tgroup/colspec (where @colnum attributes are specified) -->
      <xsl:when test="substring(ancestor::*[3]/c:colspec[@colnum=$entry.colnum]/@colname,1,7) = 'header_'">th</xsl:when>
      ...
      <xsl:when test="substring(ancestor::*[3]/c:colspec[position()=$entry.colnum and not(@colnum)]/@colname,1,7) = 'header_'">th</xsl:when>
    ...
    <xsl:variable name="provided-class">
      <xsl:choose>
        ...
        <xsl:when test="$step='colspecstep1'">
          <xsl:value-of select="normalize-space(ancestor::*[3]/c:colspec[@colnum=$entry.colnum]/@class)"/>
        </xsl:when>
        <xsl:when test="$step='colspecstep2'">
          <xsl:value-of select="normalize-space(ancestor::*[3]/c:colspec[position()=$entry.colnum and not(@colnum)]/@class)"/>
        </xsl:when>
        <xsl:when test="$step='spanspec'">
          <xsl:value-of select="normalize-space(ancestor::*[3]/c:spanspec[@spanname=current()/@spanname]/@class)"/>
        </xsl:when>
      </xsl:choose>
    </xsl:variable>
    <xsl:variable name="row.header.class.or.not">
  ...
  <xsl:template name="process.cell">
	...
	<xsl:choose>
	  <xsl:when test="not(following-sibling::*) or ancestor-or-self::*/@colsep='0' or ancestor::*/c:colspec/@colsep='0' or ancestor::*[3]/c:spanspec/@colsep='0'">
	    <xsl:call-template name="style.param.determiner">
	...
	<xsl:choose>
	  <xsl:when test="not(parent::c:row/following-sibling::c:row) or ancestor-or-self::*/@rowsep='0' or ancestor::*/c:colspec/@rowsep='0' or ancestor::*[3]/c:spanspec/@rowsep='0' or @morerows">
	    <xsl:call-template name="style.param.determiner">
	...
	<xsl:choose>
	  <xsl:when test="ancestor-or-self::*/@align!='left' or ancestor::*/c:colspec/@align!='left' or @spanname">
	    <xsl:call-template name="style.param.determiner">
      ...
      <xsl:if test="ancestor-or-self::*/@char or ancestor::*/c:colspec/@char or ancestor::*[3]/c:spanspec/@char">
	<xsl:call-template name="style.param.determiner">
	  <xsl:with-param name="style.name" select="'char'"/>
	</xsl:call-template>
      </xsl:if>
      <xsl:if test="ancestor-or-self::*/@charoff or ancestor::*/c:colspec/@charoff or ancestor::*[3]/c:spanspec/@charoff">
	<xsl:call-template name="style.param.determiner">
  ...
  <xsl:template name="style.param.determiner">
    ...
    <xsl:choose>
      ...
      <xsl:when test="$style.name='rowsep' and          (parent::c:row[not(following-sibling::c:row)]           and not(ancestor::*[2][preceding-sibling::c:tfoot or self::c:thead]      or (ancestor::c:tgroup[following-sibling::c:tgroup] and not(ancestor::c:entrytbl))) or          @morerows=count(parent::c:row/following-sibling::c:row)           and not(ancestor::*[2][preceding-sibling::c:tfoot or self::c:thead]      or (ancestor::c:tgroup[following-sibling::c:tgroup] and not(ancestor::c:entrytbl))))">
	<xsl:call-template name="style.maker">
      ...
      <xsl:when test="$style.name='colsep' and          not(following-sibling::*) and          not(parent::c:row/preceding-sibling::c:row/c:entry[position()=last()][(@morerows + count(parent::c:row/preceding-sibling::c:row)) &gt;= count(current()/parent::c:row/preceding-sibling::c:row)])">
	<xsl:call-template name="style.maker">
	  <xsl:with-param name="style.param" select="'0'"/>
	  <xsl:with-param name="style.name" select="'colsep'"/>
	</xsl:call-template>
	...
	<xsl:call-template name="style.maker">
      ...
      <xsl:when test="$style.name='rowsep' and parent::c:row/@rowsep">
	<xsl:call-template name="style.maker">
	  <xsl:with-param name="style.param" select="parent::c:row/@rowsep"/>
	  <xsl:with-param name="style.name" select="'rowsep'"/>
	</xsl:call-template>
      </xsl:when>
      ...
      <xsl:when test="ancestor::*[2]/c:colspec[@colnum=$entry.colnum]/attribute::*[local-name()=$style.name]">
	<xsl:call-template name="style.maker">
	  <xsl:with-param name="style.param" select="ancestor::*[2]/c:colspec[@colnum=$entry.colnum]/attribute::*[local-name()=$style.name]"/> 
	  <xsl:with-param name="style.name" select="$style.name"/>
	</xsl:call-template>
      </xsl:when>
      ...
      <xsl:when test="ancestor::*[2]/c:colspec[position()=$entry.colnum and not(@colnum)]/attribute::*[local-name()=$style.name]">
	<xsl:call-template name="style.maker">
	  <xsl:with-param name="style.param" select="ancestor::*[2]/c:colspec[position()=$entry.colnum and not(@colnum)]/attribute::*[local-name()=$style.name]"/>
	  <xsl:with-param name="style.name" select="$style.name"/>
	</xsl:call-template>
      </xsl:when>
      <!-- nearest matching spanspec (if a thead or tfoot have colspecs, however, the tgroup or entrytbl's colspecs are ignored) -->
      <xsl:when test="ancestor::*[3]/c:spanspec[@spanname=current()/@spanname]/attribute::*[local-name()=$style.name] and not(ancestor::*[2]/c:colspec)">
	<xsl:call-template name="style.maker">
	  <xsl:with-param name="style.param" select="ancestor::*[3]/c:spanspec[@spanname=current()/@spanname]/attribute::*[local-name()=$style.name]"/>
	  <xsl:with-param name="style.name" select="$style.name"/>
	</xsl:call-template>
      </xsl:when>
      <!-- nearest entrytbl/colspec or tgroup/colspec (where @colnum attributes are specified) -->
      <xsl:when test="ancestor::*[3]/c:colspec[@colnum=$entry.colnum]/attribute::*[local-name()=$style.name] and not(ancestor::*[2]/c:colspec)">
	<xsl:call-template name="style.maker">
	  <xsl:with-param name="style.param" select="ancestor::*[3]/c:colspec[@colnum=$entry.colnum]/attribute::*[local-name()=$style.name]"/>
	  <xsl:with-param name="style.name" select="$style.name"/>
	</xsl:call-template>
      </xsl:when>
      <!-- nearest entrytbl/colspec or tgroup/colspec (where no @colnum attributes are specified and colspecs are instead ordered sequentially) -->
      <xsl:when test="ancestor::*[3]/c:colspec[position()=$entry.colnum and not(@colnum)]/attribute::*[local-name()=$style.name] and not(ancestor::*[2]/c:colspec)">
	<xsl:call-template name="style.maker">
	  <xsl:with-param name="style.param" select="ancestor::*[3]/c:colspec[position()=$entry.colnum]/attribute::*[local-name()=$style.name]"/>
	  <xsl:with-param name="style.name" select="$style.name"/>
	</xsl:call-template>
      </xsl:when>
	...
	<xsl:call-template name="style.maker">
      ...
      <xsl:when test="ancestor::c:table/attribute::*[local-name()=$style.name] and not(ancestor::c:entrytbl)">
	<xsl:call-template name="style.maker">
	  <xsl:with-param name="style.param" select="ancestor::c:table/attribute::*[local-name()=$style.name]"/>
	  <xsl:with-param name="style.name" select="$style.name"/>
	</xsl:call-template>
      </xsl:when>
	...
	<xsl:choose>
	  ...
	  <xsl:when test="$style.name='align'">
	    <xsl:choose>
	      ...
	      <xsl:when test="@spanname and not(ancestor::*[2]/c:colspec)">
		<xsl:call-template name="style.maker">
  ...
  <!-- This template adds border styles, alignments, and char-related attributes to the HTML for the current entry(tbl), depending on 
       how those attributes were determined in style.param.determiner (or in the process.cell/c:entrytbl templates if the current 
  ...
  <xsl:template name="entry.colnum">
    <xsl:param name="entry" select="."/>
    <xsl:choose>
      <xsl:when test="$entry/ancestor::*[3]/c:spanspec[@spanname=$entry/@spanname] and not($entry/ancestor::*[2]/c:colspec)">
	<xsl:variable name="namest" select="$entry/ancestor::*[3]/c:spanspec[@spanname=$entry/@spanname]/@namest"/>
	<xsl:call-template name="colspec.colnum">
	  <xsl:with-param name="colspec" select="$entry/ancestor::*[3]/c:colspec[@colname=$namest]"/>
	</xsl:call-template>
      </xsl:when>
      <xsl:when test="$entry/ancestor::*[2]/c:colspec[@colname=$entry/@namest]">
	<xsl:call-template name="colspec.colnum">
	  <xsl:with-param name="colspec" select="$entry/ancestor::*[2]/c:colspec[@colname=$entry/@namest]"/>
	</xsl:call-template>
      </xsl:when>
      <xsl:when test="$entry/ancestor::*[2]/c:colspec[@colname=$entry/@colname]">
	<xsl:call-template name="colspec.colnum">
	  <xsl:with-param name="colspec" select="$entry/ancestor::*[2]/c:colspec[@colname=$entry/@colname]"/>
	</xsl:call-template>
      </xsl:when>
      <xsl:when test="$entry/ancestor::*[3]/c:colspec[@colname=$entry/@namest] and not($entry/ancestor::*[2]/c:colspec)">
	<xsl:call-template name="colspec.colnum">
	  <xsl:with-param name="colspec" select="$entry/ancestor::*[3]/c:colspec[@colname=$entry/@namest]"/>
	</xsl:call-template>
      </xsl:when>
      <xsl:when test="$entry/ancestor::*[3]/c:colspec[@colname=$entry/@colname] and not($entry/ancestor::*[2]/c:colspec)">
	<xsl:call-template name="colspec.colnum">
	  <xsl:with-param name="colspec" select="$entry/ancestor::*[3]/c:colspec[@colname=$entry/@colname]"/>
	</xsl:call-template>
      </xsl:when>
      <xsl:when test="count($entry/../*) = $entry/ancestor::*[3]/@cols">
	<xsl:value-of select="count($entry/preceding-sibling::*) + 1"/>
      </xsl:when>
      <xsl:when test="$entry/parent::c:row/preceding-sibling::c:row/c:entry[(@morerows + count(parent::c:row/preceding-sibling::c:row)) &gt;= count($entry/parent::c:row/preceding-sibling::c:row)]">
  ...
  <xsl:template name="entry.ending.colnum">
    ...
    <xsl:choose>
      <xsl:when test="$entry/ancestor::*[3]/c:spanspec[@spanname=$entry/@spanname] and not($entry/ancestor::*[2]/c:colspec)">
	<xsl:variable name="nameend" select="$entry/ancestor::*[3]/c:spanspec[@spanname=$entry/@spanname]/@nameend"/>
	<xsl:call-template name="colspec.colnum">
	  <xsl:with-param name="colspec" select="$entry/ancestor::*[3]/c:colspec[@colname=$nameend]"/>
	</xsl:call-template>
      </xsl:when>
      <xsl:when test="$entry/ancestor::*[2]/c:colspec[@colname=$entry/@nameend]">
	<xsl:call-template name="colspec.colnum">
	  <xsl:with-param name="colspec" select="$entry/ancestor::*[2]/c:colspec[@colname=$entry/@nameend]"/>
	</xsl:call-template>
      </xsl:when>
      <xsl:when test="$entry/ancestor::*[2]/c:colspec[@colname=$entry/@colname]">
	<xsl:call-template name="colspec.colnum">
      ...
      <xsl:when test="$entry/ancestor::*[3]/c:colspec[@colname=$entry/@nameend] and not($entry/ancestor::*[2]/c:colspec)">
	<xsl:call-template name="colspec.colnum">
	  <xsl:with-param name="colspec" select="$entry/ancestor::*[3]/c:colspec[@colname=$entry/@nameend]"/>
	</xsl:call-template>
      </xsl:when>
      <xsl:when test="$entry/ancestor::*[3]/c:colspec[@colname=$entry/@colname] and not($entry/ancestor::*[2]/c:colspec)">
	<xsl:call-template name="colspec.colnum">
  ...
  <xsl:template name="morerows.check">
    ...
    <xsl:choose>
      <xsl:when test="generate-id($mc.entry/ancestor::*[2]/c:row[position()=$mc.row.number.being.checked]/child::*[position()=$mc.entry.number.being.checked])                 = generate-id($mc.entry)">
  ...
  <xsl:template name="mc.cols.assignment">
    ...
    <xsl:param name="mca.entry.being.checked" select="$mc.entry/ancestor::*[2]/c:row[position()=$mc.row.number.being.checked]/child::*[position()=$mc.entry.number.being.checked]"/>
	...
	<!-- If the entry we're on in morerows.check has a colspan, also assign the other corresponding colnums. -->
	  ...
	  <xsl:otherwise>
	    <xsl:choose>
	      ...
	      <xsl:when test="boolean($mc.entry/ancestor::*[2]/c:row[position()=$mc.row.number.being.checked]/child::*[position()=$mc.entry.number.being.checked][following-sibling::*])">
		<xsl:call-template name="morerows.check">
		  <xsl:with-param name="mc.row.number.being.checked" select="$mc.row.number.being.checked"/>
  ...
  <xsl:template name="col.maker">
    <xsl:param name="cm.iteration" select="'1'"/>
      ...
      <xsl:choose>
	<xsl:when test="child::*/c:colspec[(@colnum=$cm.iteration) or (position()=$cm.iteration and not(@colnum))]/@colwidth">
	  <xsl:value-of select="child::*/c:colspec[(@colnum=$cm.iteration) or (position()=$cm.iteration and not(@colnum))]/@colwidth"/>
	</xsl:when>
	<xsl:when test="c:colspec[(@colnum=$cm.iteration) or (position()=$cm.iteration and not(@colnum))]/@colwidth">
	  <xsl:value-of select="c:colspec[(@colnum=$cm.iteration) or (position()=$cm.iteration and not(@colnum))]/@colwidth"/>
	</xsl:when>
      </xsl:choose>
    </xsl:param>
    <xsl:choose>
	...
	<col>
	  ...
	  <xsl:choose>
	    <xsl:when test="c:colspec[(@colnum=$cm.iteration) or (position()=$cm.iteration and not(@colnum))][@colwidth!=''] or         child::*/c:colspec[(@colnum=$cm.iteration) or (position()=$cm.iteration and not(@colnum))][@colwidth!='']">
