...
do
  ...
  test ${BATCH} -eq 0 && xsltproc ${ELEMENTS_XSL} ${f}.xml 2>&1 | sort | uniq -c | sort -n -r
  # Print out just a "richness" number, meaning a count of all the "uncommon" elements
    COMMON=$(xsltproc ${ELEMENTS_XSL} ${f}.xml 2>&1 | grep "__misc__" | wc -l)
  UNCOMMON=$(xsltproc ${ELEMENTS_XSL} ${f}.xml 2>&1 | grep -v "__misc__" | wc -l)
